...
const validNumber = v => /^\d{7,15}$/.test(digits(v));
...
async function getWhatsAppVersion() {
  try {
    if (typeof fetchLatestWaWebVersion === 'function') {
      ...
      if (r && Array.isArray(r.version)) {
        return r.version;
      }
    }
  } catch (e) {
    console.warn(
      'Live WA Web version lookup failed:',
      e.message
    );
  }
...
async function createSocket(number, existingSession) {
  ...
  if (entry?.sock && !entry.restarting) {
    return entry;
  }
...
After entering the code, keep this page open
until it says LOGIN SUCCESS.
...
$('pc').onclick =
  async () => {
    ...
    if(
      ...
      return say(
        'Number invalid hai. Country code ke saath number dalo.',
        'bad'
      );
    ...
    try{
      ...
      say(
        'Code WhatsApp → Linked devices → Link with phone number mein enter karo.',
        'good'
      );
