  ...
  sock.ev.on('connection.update', async update => {
    ...
    if (connection === 'close') {
      ...
      if (entry.reconnects < 8) {
        const wait = Math.min(
          15000,
          1500 * Math.pow(1.5, entry.reconnects)
        );

        entry.reconnects++;

        setTimeout(
          () => restartSocket(number, entry),
          wait
        );
      } else {
        entry.status = 'failed';
        entry.error =
          'Connection could not be recovered. Generate a new code.';
      }
    }
  });
...
.active,.primary{
background:linear-gradient(135deg,#7047ff,#a16cff);
border:0
}
.box{
margin-top:18px;
...
<button id="qrgo" class="primary">
Generate QR
</button>

<div id="qb" class="box" hidden>
<b>SCAN WITH WHATSAPP</b>
<br>
<img id="qr" class="qr">
</div>
...
<button id="pc" class="primary">
Generate Pair Code
</button>

<div id="pb" class="box" hidden>
...
<div class="note">
Use your own WhatsApp account.
After entering the code, keep this page open until it says LOGIN SUCCESS.
</div>
...
$('pc').onclick=async()=>{
  ...
  if(!/^\d{7,15}$/.test(number)){
    ...
    return say(
      'Enter a valid number with country code, digits only.',
      'bad'
    );

  }

  say('Generating pair code…');
...
$('check').onclick=async()=>{
  ...
  if(!number)return say('Start login first');
  ...
  say(
    'Status: '+x.status+
    (x.credsSaved?' • session saved':''),
    x.status==='connected'?'good':''
  );
...
app.post('/api/pair', async (req, res) => {
  ...
  if (!validNumber(number)) {
    ...
    return res.status(400).json({
      ok: false,
      error:
        'Invalid number. Use country code + number, digits only.'
    });
  ...
  try {
    ...
    if (
      !entry.sock ||
      entry.sock.ws?.readyState !== 1
    ) {

      throw new Error(
        'WhatsApp socket is not ready for pairing. Generate a new code.'
      );
    ...
    const code =
      await entry.sock.requestPairingCode(number);
...
app.post('/api/qr', async (req, res) => {
  ...
  try {
    ...
    if (!entry.qr) {
      ...
      throw new Error(
        entry.error ||
        'QR not received; check Render logs.'
      );
...
app.listen(
  ...
  () =>
    console.log(
      'Danish Khan WA Login running on port ' +
      PORT
    )
);
